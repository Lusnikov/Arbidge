<?php
    ...
    $found = "%" . $_GET['music-name'] ."%";

    $query = $pdo->prepare("SELECT * FROM Albums WHERE album_name LIKE ?");
    $query->execute(array($found));
    $albums = $query->fetchAll(PDO::FETCH_ASSOC);

    $query = $pdo->prepare("SELECT * FROM musiclist WHERE music_name LIKE ?");
    $query->execute(array($found));
...
<header class="header ">
    ...
    <main>
        <section class="main" style="overflow: hidden;">
           <h2 class="title Container" style="padding: 10px; ">Результаты поиска по альбомам</h2>    
           ...
           <div class="album-list">
            ...
            <h2 class="title Container" style="padding: 10px; ">Результаты поиска по аудио</h2>   
